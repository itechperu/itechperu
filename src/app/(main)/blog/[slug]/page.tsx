...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const renderContent = (content: string) => {
    return content.split("\n\n").map((block, i) => {
      if (block.startsWith("## ")) {
        return <h2 key={i} className="text-[18px] font-bold text-[#1D1D1F] mt-6 mb-2">{block.replace("## ", "")}</h2>;
      }
      if (block.startsWith("### ")) {
        return <h3 key={i} className="text-[15px] font-semibold text-[#1D1D1F] mt-4 mb-1">{block.replace("### ", "")}</h3>;
      }
      return <p key={i} className="text-[14px] text-[#1D1D1F]/80 leading-relaxed mb-3">{block}</p>;
    });
  };
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-10 py-8 lg:py-12">
      ...
      <h1 className="text-[24px] lg:text-[32px] font-bold tracking-tight text-[#1D1D1F]">{post.title}</h1>
      ...
      <div className="mt-8 rounded-3xl bg-gradient-to-br from-[#1D1D1F] to-[#2A2A2D] p-6 text-center">
        ...
        <a href="https://wa.me/51987654321" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 mt-3 rounded-full bg-[#D4AF37] px-5 py-2 text-[12px] font-semibold text-[#1D1D1F]">
      ...
      <div className="mt-8">
        <h2 className="text-[16px] font-bold text-[#1D1D1F] mb-3">Artículos relacionados</h2>
        <div className="grid sm:grid-cols-2 gap-3">
          {BLOG_POSTS.filter((p) => p.slug !== post.slug).slice(0, 2).map((p) => (
            <Link key={p.slug} href={`/blog/${p.slug}`} className="group rounded-2xl bg-white border border-[#E5E5E7] p-4 hover:shadow-md transition-all">
              <span className="text-[10px] text-[#D4AF37] font-medium">{p.category}</span>
              <h3 className="text-[13px] font-semibold text-[#1D1D1F] mt-1 group-hover:text-[#D4AF37] transition-colors">{p.title}</h3>
              <span className="mt-2 inline-flex items-center gap-1 text-[11px] text-[#86868B]">Leer <ChevronRight className="h-3 w-3" strokeWidth={2} /></span>
            </Link>
          ))}
        </div>
      </div>
