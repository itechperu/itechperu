...
export default function BlogPage() {
  return (
    <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-10 py-8 lg:py-12">
      ...
      <h1 className="text-[28px] lg:text-[40px] font-bold tracking-tight text-[#1D1D1F]">Blog itechperu</h1>
      <p className="mt-2 text-[14px] text-[#86868B]">Guías expertas sobre tecnología, reparación y compras inteligentes en Perú</p>

      <div className="mt-8 grid sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
        {BLOG_POSTS.map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group rounded-3xl bg-white border border-[#E5E5E7] overflow-hidden hover:shadow-lg transition-all">
            ...
            <div className="p-4 lg:p-5">
              <div className="flex items-center gap-2 text-[10px] text-[#86868B] mb-2">
              ...
              <h2 className="text-[15px] lg:text-[16px] font-semibold text-[#1D1D1F] group-hover:text-[#D4AF37] transition-colors">{post.title}</h2>
              <p className="mt-1 text-[12px] text-[#86868B] line-clamp-2">{post.excerpt}</p>
              <span className="mt-3 inline-flex items-center gap-1 text-[12px] font-medium text-[#1D1D1F] group-hover:text-[#D4AF37] transition-colors">
