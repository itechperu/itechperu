      ...
      {isOpen && (
        <>
          ...
          <motion.aside
            ...
            className="fixed right-0 top-0 bottom-0 z-[100] w-full max-w-md bg-white shadow-[0_0_60px_rgb(0,0,0,0.2)] flex flex-col"
            role="dialog"
            aria-label="Carrito de compras"
          >
            {/* Header */}
            <header className="flex items-center justify-between px-5 py-4 border-b border-[#E5E5E7]">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h2 className="text-[16px] font-bold tracking-tight text-[#1D1D1F]">
                    Tu Carrito
                  </h2>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4">
              {items.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center">
                  <div className="flex h-16 w-16 items-center justify-center rounded-full bg-[#F5F5F7] mb-3">
                    <ShoppingBag className="h-7 w-7 text-[#86868B]" strokeWidth={1.5} />
                  </div>
                  <p className="text-[14px] font-semibold text-[#1D1D1F]">
                    Tu carrito está vacío
                  </p>
                  <p className="text-[12px] text-[#86868B] mt-1 max-w-[200px]">
                ...
                <ul className="space-y-3">
                  {items.map((item) => (
                    <motion.li
                      ...
                      exit={{ opacity: 0, x: -20 }}
                      layout
                      className="flex gap-3 rounded-2xl bg-[#F5F5F7] p-3"
                    >
                      {/* Image */}
                      <div className="flex-shrink-0 h-16 w-16 rounded-xl overflow-hidden bg-white">
                        <img
                          src={item.image}
                          alt={item.title}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-[12px] font-semibold text-[#1D1D1F] leading-tight line-clamp-2">
                          {item.title}
                        </p>
                        ...
                        <div className="flex items-center justify-between mt-2">
                          <div className="flex items-center gap-1 rounded-full bg-white p-0.5">
                            <button
                              ...
                              className="flex h-6 w-6 items-center justify-center rounded-full hover:bg-[#F5F5F7] transition-colors"
                              aria-label="Disminuir"
                            >
                              <Minus className="h-3 w-3 text-[#1D1D1F]" strokeWidth={2} />
                            </button>
                            <span className="w-6 text-center text-[12px] font-semibold text-[#1D1D1F]">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() =>
                              ...
                              aria-label="Aumentar"
                            >
                              <Plus className="h-3 w-3 text-[#1D1D1F]" strokeWidth={2} />
                            </button>
                          </div>

                          <button
                            onClick={() => removeItem(item.productId, item.grade)}
                            className="text-[#86868B] hover:text-[#EF4444] transition-colors p-1"
                            aria-label="Eliminar"
                          >
            ...
            {/* Footer con totales */}
            {items.length > 0 && (
              <footer className="border-t border-[#E5E5E7] px-5 py-4 space-y-3">
                ...
                {subtotal < 150000 && subtotal > 0 && (
                  <p className="text-[10px] text-[#86868B] text-center bg-[#F5F5F7] rounded-lg py-1.5 px-2">
